<template>
  <div class="space-y-6">
    ...
    <div class="bg-gradient-to-r from-purple-50 to-pink-50 p-6 rounded-lg">
      <h3 class="text-lg font-semibold text-gray-900 mb-4">{{ t('numberBaseConverter.title') }}</h3>
      
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('numberBaseConverter.inputValue') }}</label>
            <input
              v-model="inputValue"
              :placeholder="t('numberBaseConverter.enterValue')"
          ...
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('numberBaseConverter.fromBase') }}</label>
            <select
              v-model="fromBase"
              class="w-full p-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-purple-500 focus:border-purple-500"
            >
              <option value="2">{{ t('numberBaseConverter.bases.binary') }}</option>
              <option value="8">{{ t('numberBaseConverter.bases.octal') }}</option>
              <option value="10">{{ t('numberBaseConverter.bases.decimal') }}</option>
              <option value="16">{{ t('numberBaseConverter.bases.hexadecimal') }}</option>
            </select>
          </div>
        </div>
        ...
        <div class="space-y-4">
          <div>
            <label class="block text-sm font-medium text-gray-700 mb-2">{{ t('numberBaseConverter.toBase') }}</label>
            <select
              v-model="toBase"
    ...
    <div v-if="conversionResult" class="space-y-4">
      <h3 class="text-lg font-semibold text-gray-900">{{ t('numberBaseConverter.conversionResult') }}</h3>
      ...
      <div class="flex space-x-2">
        ...
        >
          {{ t('numberBaseConverter.copyResult') }}
        </button>
        <button
          @click="copyAll"
          class="px-4 py-2 bg-green-500 text-white rounded-lg hover:bg-green-600 transition-colors"
        >
          {{ t('numberBaseConverter.copyAll') }}
        </button>
      </div>
    </div>
    ...
    <div v-if="allBasesResult" class="space-y-4">
      <h3 class="text-lg font-semibold text-gray-900">{{ t('numberBaseConverter.allBasesRepresentation') }}</h3>
...
const quickConversions = computed(() => [
  { name: t('numberBaseConverter.quickConversions.decimalToBinary'), value: '255', fromBase: '10', toBase: '2', description: t('numberBaseConverter.quickConversions.decimalToBinaryDesc') },
  { name: t('numberBaseConverter.quickConversions.hexToDecimal'), value: 'FF', fromBase: '16', toBase: '10', description: t('numberBaseConverter.quickConversions.hexToDecimalDesc') },
  { name: t('numberBaseConverter.quickConversions.decimalToOctal'), value: '100', fromBase: '10', toBase: '8', description: t('numberBaseConverter.quickConversions.decimalToOctalDesc') },
  { name: t('numberBaseConverter.quickConversions.binaryToHex'), value: '1010', fromBase: '2', toBase: '16', description: t('numberBaseConverter.quickConversions.binaryToHexDesc') }
])

const getBaseName = (base) => {
  const names = {
    '2': t('numberBaseConverter.bases.binary'),
    '8': t('numberBaseConverter.bases.octal'),
    '10': t('numberBaseConverter.bases.decimal'),
    '16': t('numberBaseConverter.bases.hexadecimal')
  }
...
const convert = () => {
  ...
  try {
    ...
    if (isNaN(decimal)) {
      alert(t('numberBaseConverter.invalidInput'))
      return
    }
    ...
    alert(t('numberBaseConverter.conversionFailed') + ': ' + e.message)
  }
}
