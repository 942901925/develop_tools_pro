<template>
  <div class="space-y-6">
    <!-- 输入区域 -->
    <div class="space-y-4">
      <h3 class="text-lg font-semibold text-gray-900">{{ t('textStatistics.textInput') }}</h3>
      <textarea
        v-model="inputText"
    ...
    <div v-if="inputText" class="space-y-6">
      ...
      <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
        <div class="bg-blue-50 p-4 rounded-lg text-center">
          ...
          <div class="text-sm text-gray-600">{{ t('textStatistics.characters') }}</div>
        </div>
        <div class="bg-green-50 p-4 rounded-lg text-center">
          <div class="text-2xl font-bold text-green-600">{{ statistics.words }}</div>
          <div class="text-sm text-gray-600">{{ t('textStatistics.words') }}</div>
        </div>
        <div class="bg-purple-50 p-4 rounded-lg text-center">
          <div class="text-2xl font-bold text-purple-600">{{ statistics.lines }}</div>
          <div class="text-sm text-gray-600">{{ t('textStatistics.lines') }}</div>
        </div>
        <div class="bg-orange-50 p-4 rounded-lg text-center">
          <div class="text-2xl font-bold text-orange-600">{{ statistics.paragraphs }}</div>
          <div class="text-sm text-gray-600">{{ t('textStatistics.paragraphs') }}</div>
        </div>
      </div>
      ...
      <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
        <!-- 字符类型统计 -->
        <div class="space-y-4">
          <h4 class="text-lg font-semibold text-gray-900">{{ t('textStatistics.charTypeStats') }}</h4>
          <div class="space-y-2">
            <div class="flex justify-between">
              <span>{{ t('textStatistics.letters') }}</span>
              <span class="font-mono">{{ statistics.letters }}</span>
            </div>
            <div class="flex justify-between">
              <span>{{ t('textStatistics.digits') }}</span>
              <span class="font-mono">{{ statistics.digits }}</span>
            </div>
            <div class="flex justify-between">
              <span>{{ t('textStatistics.spaces') }}</span>
              <span class="font-mono">{{ statistics.spaces }}</span>
            </div>
            <div class="flex justify-between">
              <span>{{ t('textStatistics.punctuation') }}</span>
              <span class="font-mono">{{ statistics.punctuation }}</span>
            </div>
            <div class="flex justify-between">
              <span>{{ t('textStatistics.specialChars') }}</span>
        ...
        <!-- 单词统计 -->
        <div class="space-y-4">
          <h4 class="text-lg font-semibold text-gray-900">{{ t('textStatistics.wordStats') }}</h4>
          <div class="space-y-2">
            <div class="flex justify-between">
              <span>{{ t('textStatistics.avgWordLength') }}</span>
              <span class="font-mono">{{ statistics.avgWordLength }}</span>
            </div>
            <div class="flex justify-between">
              <span>{{ t('textStatistics.longestWord') }}</span>
              <span class="font-mono text-sm">{{ statistics.longestWord }}</span>
            </div>
            <div class="flex justify-between">
              <span>{{ t('textStatistics.shortestWord') }}</span>
              <span class="font-mono text-sm">{{ statistics.shortestWord }}</span>
            </div>
            <div class="flex justify-between">
              <span>{{ t('textStatistics.uniqueWords') }}</span>
      ...
      <div v-if="topWords.length > 0" class="space-y-4">
        <h4 class="text-lg font-semibold text-gray-900">{{ t('textStatistics.topWords') }}</h4>
        <div class="grid grid-cols-1 md:grid-cols-2 gap-2">
          ...
          >
            ...
            <span class="text-sm text-gray-600">{{ word.count }}{{ t('textStatistics.times') }}</span>
          </div>
        </div>
      </div>
      ...
      <!-- 文本预览 -->
      <div class="space-y-4">
        <h4 class="text-lg font-semibold text-gray-900">{{ t('textStatistics.textPreview') }}</h4>
        <div class="p-4 bg-gray-100 rounded-lg">
          <div class="text-sm text-gray-600 mb-2">{{ t('textStatistics.first100Chars') }}</div>
